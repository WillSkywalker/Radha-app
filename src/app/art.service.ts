...
export class ArticleService {
  ...
  getArticles(): Promise<ArticlePre[]> {
    return this.http.get(this.ApiUrl+'article', {params: {"sort": 'viewcount'}}).toPromise()
  ...
  getRecentArticles(): Promise<ArticlePre[]> {
    return this.http.get(this.ApiUrl+'article').toPromise()
        .then(res => res.json() as ArticlePre[])
        .catch(this.handleError);
  }

  getHottestArticles(): Promise<ArticlePre[]> {
    return Promise.resolve(ARTICLES.slice(-3).reverse());
  }

  getRecommendedArticles(): Promise<ArticlePre[]> {
    return Promise.resolve(ARTICLES.slice(-3).reverse());
  }
  ...
  getArticlePreview(id: number): Promise<ArticlePre> {
    return this.getArticles()
      .then(articles => articles.find(art => art.id === id));
  }
